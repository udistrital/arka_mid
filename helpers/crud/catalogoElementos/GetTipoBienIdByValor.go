...
func GetTipoBienIdByValor(ctx context.Context, tbPadreId int, normalizado float64, bufferTiposBien map[int]models.TipoBien) (tipoBienId int, outputError map[string]interface{}) {
	...
	if err := GetAllTipoBien(ctx, payload, &tb__); err != nil || len(tb__) != 1 {
		return 0, err
	}
